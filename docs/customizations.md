...
## Last upgrade — v0.4.9 → v0.4.11 (CODE-COMPLETE 2026-07-27, Phase 6 deploy PENDING user confirmation)

**Code complete through Phase 5** — merge `185d446d8`, base now `v0.4.11` (`git describe` = `v0.4.11-221-g185d446d8`). Clean merge-base `ecbdbda09` (= v0.4.9), 35 upstream commits (v0.4.9→v0.4.11 two-step combined). **0 content conflicts** (merge-tree preview predicted 1 candidate `docker-compose.selfhost.build.yml`; rerere auto-resolved it). Verify green: pnpm install / typecheck(6/6) / go build / pnpm test(views **3075**) / pnpm build(3/3); go test migrations+issueguard green.

**Headline — fork 213/214 archived migrations collide-by-number with upstream 213/214** (5th/6th duplicate-prefix collision). fork's ahead-of-release VCS/Grok backports (MUL-3772/5228, #5868/#5883) are byte-identical to the official v0.4.11 commits → merged clean, live `schema_migrations` already matches final numbering → **DB has 0 pending migrations**. Actions taken: (1) added `"213"`/`"214"` to `legacyDuplicateMigrationStems` (lint red→green); (2) re-resolved the compose file because rerere's recorded resolution was **stale** — it re-added `REMOTE_API_URL`/`NEXT_PUBLIC_WS_URL` build args that upstream removed (`a90aa92d0` runtime URL resolution; `Dockerfile.web` no longer has those ARGs) — kept fork's required `VERSION`/`APP_VERSION` `:?` guards, dropped the dead args. **Lesson: rerere hit ≠ semantically correct — always re-verify.**
...
**Phase 6 (deploy) PENDING user confirmation.** When authorized: (1) pg17 `pg_dump` backup (confirmed yes); (2) migrate up — expect 0 applied; (3) re-stamp version frontend `.env NEXT_PUBLIC_APP_VERSION=v0.4.11` + backend `-ldflags -X main.version=v0.4.11`; (4) restart via `launchctl kickstart -k` (avoid install.sh two defects — see memory `project_install_sh_reload_defects`); (5) health check `/healthz`+`/` (backend ~40s init — first probe may be 000). Then flip plan gates to done + Phase 7 record.

The full per-file strategy, verified negative claims, and Phase-by-Phase record:

→ **`docs/upgrades/v0.4.11-plan.md`** (frontmatter `upgrade_contract: selfhost-upgrade/v1`, `phase: gates`)
