...
*Audit 2026-08-07:* **✅ v0.4.18 → v0.4.20 upgrade EXECUTED 2026-08-07** (merge commit `e4b0277ba`, a TRUE 2-parent merge — **corrects the v0.4.19 fake-merge**: `c03402a94`/`c95bb7e89` were single-parent linear commits that never merged v0.4.19's tree, so the real jump was v0.4.18→v0.4.20, **61 commits / 603 files**). **42 手动冲突**（超过 Phase 1 预测的 37 — locale JSON 的 hunk 级冲突多于文件级预估）— 根因：**upstream Mika 重写 (#6378) 删 fork onboarding 模板 + MUL-5703 术语收敛（upstream 采纳 fork 的 issue→任务 + 全角标点）+ MUL-5442 reply 策略分叉 + server Strategy D 签名碰撞（router.go createIssue/enqueueMedia 7→9 参, task.go enqueueIssueTask/CompleteTask/FailTask) + issue-detail pin-comment 双机制**。后端冲突面真实存在（router.go/task.go/execenv 真撞）;**4 个新迁移 (257/258/259/260)** 全 > fork max 256 → 零撞号，6 已知撞号 (119/158/159/160/213/214) 完好。Phase 5 抓住 14 个测试失败（fork-code × upstream-test 3-way 不可见碰撞）:issue-detail badge 唯一性 + locale parity EN 覆盖。**本轮教训**: `--theirs` 整文件覆盖会抹 fork-only 成员（locale key/client.ts 方法）→ 改用 `git merge-file` 三方;fork 定制是跨代码+测试+每 locale 的不变量集合;parity "EN covers" 同时是 fork-only-key 探测器（既找出活 key 需补 EN，也找出 dead key 如 token-usage 侧边栏 — upstream 已删，删 key 而非复活）。**install.sh commit-first 顺序正确**（先 merge commit 再 install.sh,`git describe` 解析 v0.4.20，无需手动兜底）;**daemon 漂移清除**(restart 0.4.19→0.4.20,CLI/daemon/backend 全对齐）。Per-file strategy, 75 条 verified negative claims, actuals-vs-plan live in `docs/upgrades/v0.4.20-plan.md` (see "Last upgrade").
