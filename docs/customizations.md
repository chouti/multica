...
*Audit 2026-08-07:* **✅ v0.4.20 → v0.4.21 upgrade EXECUTED 2026-08-07** (merge commit `55bf3432f`, TRUE 2-parent). Clean merge-base `93342d04a` (= v0.4.20 tag), **29 upstream commits / 315 files (+19892/-2761)**. Drivers: **WeCom 渠道 (MUL-5226) + Analytics 重命名 (MUL-5759)**. **6 手动冲突 == merge-tree 预测完全吻合**（README + mention-suggestion.tsx×3 + editor.json×4）。**后端零 Strategy D scenario A 双签名碰撞**（dreaded funcs 全 byte-identical;enqueueMedia + generateIssuePrefix 是 scenario B 单边干净）。冲突要点：**mention-suggestion.tsx (C+D)** 保 fork typeGroups 注册表分组 + 补 upstream cancelled 分层（删 upstream 冗余 users/issues 数组）;**editor.json×4** manual union + 逗号修复（fork skill_* + upstream group_cancelled);**README** 取 upstream MUL-5812 重写。语义陷阱（auto-merge 过但需人工确认）:issue.go buildSearchQuery keep-both(archived-rank ⊕ cancelledRank ⊕ 3-arg ORDER BY)、task.go truncateFallbackCommentBody 去重、cmd_issue_test.go 边界。**sqlc regen 统一 issue.sql.go**(upstream 2 查询 + fork Archived)。loss scan 全 ∅（仅 upstream 删的 usage.json dead key,接受非恢复）。**4 新迁移 261-264** 全 > fork max 260 → 零撞号；6 已知撞号完好。Phase 5 全绿（typecheck 6/6、test 3824/3824、go build/vet、build 3/3),**零 fork-code×upstream-test 碰撞**（与 C4 预测一致）。install.sh commit-first 正确（merge commit 55bf3432f → git describe 解析 v0.4.21);**daemon brew upgrade 0.4.20→0.4.21 + restart**(CLI/daemon/backend/frontend 全 0.4.21,5 workspace 完好）。Per-file strategy, verified negative claims, actuals-vs-plan live in `docs/upgrades/v0.4.21-plan.md` (see "Last upgrade").
