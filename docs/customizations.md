...
## Last upgrade — v0.4.13 → v0.4.14 (EXECUTED 2026-07-30)

✅ **EXECUTED 2026-07-30** — merge `70b39883d`, base now `v0.4.14`. Clean merge-base `v0.4.13`, 34 upstream commits. **3 content conflicts + 1 rerere + 1 auto-merge semantic fix** (vs v0.4.13's 6 drafts-cluster conflicts — this round the drafts cluster auto-merged clean; the conflict surface moved to `comment.go`'s mention-trigger path). Resolved: **comment.go** (2 blocks, squad+agent branches) — *union* of fork `if m.ID=="all"` skip (@all is a broadcast, silent not blocked) + upstream `blockTarget`+Reason (MUL-5411 malformed-id); fork and upstream had **independently fixed the same malformed-id panic**; **auth.go** — `h.userToResponse(user)` + retain fork `IsSuperAdmin` (upstream made `userToResponse` a Handler method); **multica-mentioning/SKILL.md** — keep both (upstream malformed-id note + fork @skill undesignated no-op); **table-view.tsx** — rerere auto-resolved, verified fork `formatProgressText` retained + `list-row` re-export of `child-progress` symbols valid. **1 auto-merge semantic fix** (go build caught, merge-tree missed): **admin.go:177** fork `userToResponse(updated)` → `h.userToResponse(updated)` (upstream method refactor reached fork's #4118 super-admin rename). Verify green: pnpm install / typecheck(6/6) / go build / pnpm test(views **3314**) / pnpm build(3/3).

**Headline — no new migration collision; 2 real new migrations applied.** v0.4.14 adds **233-234** (`agent_task_queue_agent_terminal_latest_index` + `agent_task_queue_retired_session_id`, both `CREATE INDEX CONCURRENTLY`) — all > fork max 232, **no new duplicate-prefix collision** (the 6 known: 119/158/159/160/213/214 stand). DB migrated 2 (low risk, pure indexes).

**task.go duplicate-symbol did NOT recur.** Three-way merge kept fork's symbol-less `task.go` (v0.4.13 had moved `truncateFallbackCommentBody` to `fallback_comment_truncate.go`); upstream v0.4.14's task.go changes (daemon/perf, lines 2754+) didn't touch the symbol region → `fallback_comment_truncate.go` (cap=200) remains sole definition. No `go build` redeclared. (v0.4.13's "restore upstream cap=8000" product question remains open.)

**Deploy (Phase 6) executed:** backed up `~/multica-backups/v0.4.14-pre-migrate-20260730-095202.dump` (41M, pg17); migrate up applied **2** (233-234); version re-stamped v0.4.14 via **commit-merge-first then install.sh** — `git describe --abbrev=0` resolves to v0.4.14 after the merge commit, so install.sh stamps both frontend (`.env NEXT_PUBLIC_APP_VERSION`) and backend (`-ldflags`) correctly (avoids v0.4.13's "install.sh stamps previous version" trap, memory `project-install-sh-version-needs-commit`); restarted via install.sh plist reload; health backend `/healthz` 200 + frontend `/` 200, `/api/config` `server_version=v0.4.14`.

**Calibration (actuals vs audit prediction):** merge-tree predicted 4 content conflicts; actual = 3 manual (table-view absorbed by **rerere**, verified non-stale) + 1 auto-merge semantic error (admin.go, invisible to merge-tree). Text-conflict prediction was accurate; semantic conflict again only caught by `go build` — consistent with v0.4.13. **New variable this round: rerere** auto-resolved table-view.tsx (benign here, but future rerere resolutions must be re-verified per SOP Step 4 caveat).

The full per-file strategy, verified negative claims, and Phase-by-Phase record:

→ **`docs/upgrades/v0.4.14-plan.md`** (frontmatter `upgrade_contract: selfhost-upgrade/v1`, `phase: done`)
