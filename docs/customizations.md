...
*Audit 2026-08-06:* **✅ v0.4.18 → v0.4.19 upgrade EXECUTED 2026-08-06** (merge in progress, base now `v0.4.19`); **14 手动冲突 (4 C careful-merge + 9 locale JSON C + 1 main_test.go hunk 内 B)** 大幅超过 v0.4.18 的 4 个 — 根因不是 upstream 重构 fork 文件，而是 **多语言翻译整体重翻 + 新 chat follow-up queue 触到 comment-input 的 fork skill-mention 链 + task.go fork 历史累积 + 9 个 zh-Hans/ja/ko × editor/issues/layout 多 hunk 真撞**; **后端冲突面扩大** (v0.4.18 后端零风险，本轮 task.go + main_test.go 真撞但 fork 已 pre-empt 风险：own file + upstream 未加 truncate 同名符号 + task.go hunk 仅 6 case 列表扩展); **4 个新迁移 (253/254/255/256)** 全 > fork max 252 → 零撞号，6 已知撞号 (119/158/159/160/213/214) 完好。adversarial verify 推翻 5 个预判（"纯增量 = 低冲突"、"fork 无 mobile 定制"、"fork 无 chat queue 定制"、"IssuesEmptyState 符号被上游碰"、"issue-identifier-badge 上游会碰"）。本轮特色：**install.sh 在 merge 未 commit 时取错 baseline tag**（`_version-stamp.sh` 走 `git describe --tags --abbrev=0`，HEAD 还没 v0.4.19 tag → 退回 v0.4.18；手动 `go build -ldflags "-X main.version=v0.4.19"` 兜底）；**launchctl bootout 后需用 `launchctl bootstrap domain-target plist-path` 重新加载**（kickstart 要求 service 已被 launchd 注册，bootout 后会 "Could not find service"）。Per-file strategy, verified negative claims (4 agents, 49 条), and actuals-vs-plan live in `docs/upgrades/v0.4.19-plan.md` (see "Last upgrade").
...
## Last upgrade — v0.4.18 → v0.4.19 (EXECUTED 2026-08-06)
