...
## Last upgrade — v0.4.11 → v0.4.12 (EXECUTED 2026-07-28)

✅ **EXECUTED 2026-07-28** — merge `7a2b7baba`, base now `v0.4.12`. Clean merge-base `v0.4.11`, 20 upstream commits. **0 content conflicts** (merge-tree predicted 0; merge --no-commit confirmed 0). **1 auto-merge semantic fix** (go build caught it): `task.go` re-added `import "unicode/utf8"` — fork had removed it when moving `truncateFallbackCommentBody` out to its own file, and upstream MUL-5268 added a similar `utf8.RuneCountInString` guard at task.go:4435; 0 text conflict but the import gap broke `go build`. Verify green: pnpm install / typecheck(6/6) / go build (after fix) / pnpm test(views **3115**) / pnpm build(3/3).

**Headline — no new migration collision; 7 real new migrations applied.** Unlike v0.4.11 (migrated 0 because collisions already landed), v0.4.12 adds **224-230** (`agent_task_session_rollout_missing` [MUL-5305] + `channel_media_pending_*` [MUL-4934 lark media ingest]) — all > fork max 223, **no new duplicate-prefix collision** (the 6 known: 119/158/159/160/213/214 stand). DB migrated 7 (ADD COLUMN DEFAULT FALSE metadata-only + new table + 2 CONCURRENTLY indexes — low risk). fork hot files all auto-merge clean, verified via merged-tree cat-file on the merge result itself: mention cluster (#5980 `allowSpaces` orthogonal to fork skill/actor mention data flow), issue-detail (thread rail `right-3` + fork pin-comment visual side-effect, non-breaking), comment.go (orthogonal-D signature coexists — fork `triggerTasksForComment(skillMentionAgents)` + upstream `mergeCommentIntoPendingTask(headSha)`), fallback_comment_truncate.go (sole definition, no duplicate-symbol), `cmd_agent_copy.go` (fork == upstream blob sha `289d00cf7`, add/add zero conflict), editor zoom-canvas rename (no broken import — fork has no consumer of the renamed modules). **Lesson: auto-merge ≠ semantic correctness — merge-tree + 4 fan-out agents + merged-tree cat-file ALL passed, yet still missed task.go's utf8 import gap; only `go build` caught it.**

**Deploy (Phase 6) executed:** backed up `~/multica-backups/v0.4.12-pre-migrate-20260728-141159.dump` (37M, pg17); migrate up applied **7** (224-230); version re-stamped v0.4.12 (frontend `.env` + `export NEXT_PUBLIC_APP_VERSION=v0.4.12; build-frontend.sh`, backend `-ldflags -X main.version=v0.4.12` → `~/.multica/backend/server`); restarted via `launchctl kickstart -k` (backend up ~10s); health backend `/healthz` 200 + frontend `/` 200, `/api/config` `server_version=v0.4.12` (Help menu correct).

The full per-file strategy, verified negative claims, and Phase-by-Phase record:

→ **`docs/upgrades/v0.4.12-plan.md`** (frontmatter `upgrade_contract: selfhost-upgrade/v1`, `phase: done`)
