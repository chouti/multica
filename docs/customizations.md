...
## Last upgrade — v0.4.12 → v0.4.13 (EXECUTED 2026-07-29)

✅ **EXECUTED 2026-07-29** — merge `e665b9395`, base now `v0.4.13`. Clean merge-base `v0.4.12`, 25 upstream commits. **6 content conflicts** (all drafts/composer cluster — fork `@skill mention` ∩ upstream MUL-5181 unified draft lifecycle + upload ownership inversion), resolved via store-level reconciliation: upstream `setDraft(string)`+`writeDraft` kept; `skillMentionAgents` field + `setDraftPayload` added (`writeDraft` patched to preserve designations across text writes, else every keystroke drops the user's @skill picks); both composers re-applied the skill-mention seams onto upstream's `useComposerSubmit`+`useCommentUploads` skeleton; `issue-detail.test.tsx` signature union; `content-editor.tsx` `getMarkdown` takes upstream side (`stripBlobUrls` removed) + fork `getSkillMentionIds` re-applied. **2 auto-merge semantic fixes** (typecheck/test caught, merge-tree + 4 agents + adversarial verify all missed): comment-card edit flow `setDraft(key,{content})`→`setDraft(key,md)`; comment-composers test 3-arg→4-arg assertion (fork onSubmit 4th param). Verify green: pnpm install / typecheck(6/6) / go build / go vet / pnpm test(views **3270**) / pnpm build(3/3).

**Headline — no new migration collision; 2 real new migrations applied.** v0.4.13 adds **231-232** (`agent_task_queue_terminal_completed_at_index` + `channel_media_pending_object_due_index`, both `CREATE INDEX CONCURRENTLY`, no FK/table/column) — all > fork max 230, **no new duplicate-prefix collision** (the 6 known: 119/158/159/160/213/214 stand). DB migrated 2 (low risk, pure indexes).

**Backend note — `truncateFallbackCommentBody` is fork orphan code (Phase 4 overturned Phase 1).** Phase 1 (tag-based audit) flagged a duplicate-symbol risk and `git rm`'d `fallback_comment_truncate.go`; Phase 4 discovered the merge adopts fork's `task.go`, which has **NO** `truncateFallbackCommentBody` call (fork routes fallback comments through `redact.Text(body)` directly, never truncating). So there is **NO duplicate**: `fallback_comment_truncate.go` (cap=200) is the **sole definition**, kept to feed `fallback_comment_truncate_test.go`. **Lesson: verify the merged worktree state, not just the tag** — `git show v0.4.13:task.go` has the symbol but the three-way merge adopted fork's deletion of it. Whether to restore upstream's safety truncation (cap=8000 + `oversizedFallbackCommentNotice`, prevents raw-stream dumps into the thread — GH #5455) is a separate product decision, tracked here.

**Deploy (Phase 6) executed:** backed up `~/multica-backups/v0.4.13-pre-migrate-20260729-104808.dump` (38M, pg17); migrate up applied **2** (231-232); version re-stamped v0.4.13 — frontend `.env NEXT_PUBLIC_APP_VERSION` + `install.sh` rebuild; backend via manual `go build -ldflags "-X main.version=v0.4.13"` because **install.sh's version resolver is git-based and stamped v0.4.12 while the merge was still uncommitted** (see memory `project-install-sh-version-needs-commit`); restarted via install.sh plist reload + `launchctl kickstart -k`; health backend `/healthz` 200 + frontend `/` 200, `/api/config` `server_version=v0.4.13` (Help menu correct).

The full per-file strategy, verified negative claims, and Phase-by-Phase record:

→ **`docs/upgrades/v0.4.13-plan.md`** (frontmatter `upgrade_contract: selfhost-upgrade/v1`, `phase: done`)
