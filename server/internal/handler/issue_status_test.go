...
func TestUnseededWorkspaceStillAcceptsBuiltInStatuses(t *testing.T) {
	...
	// Fork: archived is a pseudo-builtin accepted by Resolve (see issuestatus.Archived),
	// so ActiveKeys lists the 7 upstream built-ins plus it.
	if len(keys) != 8 || keys[len(keys)-1] != issuestatus.Archived {
		t.Errorf("ActiveKeys on an unseeded workspace = %v, want the 7 built-ins plus archived", keys)
	}
}
