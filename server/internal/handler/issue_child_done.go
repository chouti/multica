...
func (h *Handler) notifyParentOfChildDone(ctx context.Context, prev, issue db.Issue) {
	...
	// exactly like Done/Cancelled and Backlog do. (MUL-6243) Fork archived
	// is closed-not-completed, so it parks here too via the closed set.
	parentStatus := issuestatus.Effective(ctx, h.Queries, parent.WorkspaceID, parent.Status)
	if issueguard.IsClosedStatus(parentStatus) {
		return
	}
...
// "Terminal" here is exactly the fork closed set (done | cancelled | archived),
// so this delegates to the shared classifier.
