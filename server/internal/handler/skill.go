...
//
//	https://skills.sh/{owner}/{repo}/{skill-name}  → single skill import (skillName set)
//	https://skills.sh/{owner}/{repo}                → batch import (skillName empty)
//
// An empty skillName signals batch mode: the caller should import every
// SKILL.md found in the repository.
...
func importAllSkillsFromRepo(ctx context.Context, httpClient *http.Client, owner, repo string) (*batchRepoImportResult, error) {
	defaultBranch := fetchGitHubDefaultBranch(ctx, httpClient, owner, repo)
	rawPrefix := fmt.Sprintf("https://raw.githubusercontent.com/%s/%s/%s",
		url.PathEscape(owner), url.PathEscape(repo), url.PathEscape(defaultBranch))
	...
	for _, skillPath := range skillPaths {
		...
		skillMdBody, err := fetchRawFile(ctx, httpClient, skillMdURL)
		if err != nil {
		...
		dirResp, err := doGitHubAPIGet(ctx, httpClient, dirAPIURL)
		if err != nil || dirResp.StatusCode != http.StatusOK {
		...
		var allFiles []githubContentEntry
		collectGitHubFiles(ctx, httpClient, entries, &allFiles, dirAPIURL)
...
func (h *Handler) ImportSkill(w http.ResponseWriter, r *http.Request) {
	...
	if source == sourceSkillsSh {
		_, _, skillName, parseErr := parseSkillsShParts(normalized)
		if parseErr != nil {
...
func (h *Handler) ImportSkillsBatch(w http.ResponseWriter, r *http.Request) {
	...
	owner, repo, skillName, err := parseSkillsShParts(normalized)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	...
	h.importSkillsBatch(w, r, owner, repo, batchClient, workspaceUUID, creatorUUID, member.UserID.String(), workspaceID)
}
...
func (h *Handler) importSkillsBatch(w http.ResponseWriter, r *http.Request, owner, repo string, httpClient *http.Client, workspaceUUID, creatorUUID pgtype.UUID, creatorID, workspaceID string) {
	result, err := importAllSkillsFromRepo(r.Context(), httpClient, owner, repo)
	if err != nil {
	...
	if result.Truncated {
		writeError(w, http.StatusBadGateway,
			fmt.Sprintf("repository %s/%s tree is too large to scan exhaustively; batch import aborted to avoid silent data loss", owner, repo))
		return
	}
