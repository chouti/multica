...
// Fork: archived rides along for the issue-table grouping/filtering paths
// (upstream's KNOWN LIMITATION above is about custom statuses; archived is the
// fork's pseudo-builtin and must stay groupable).
var validIssueStatuses = append(issuestatus.Canonical(), issuestatus.Archived)
...
func buildSearchQuery(phrase string, terms []string, queryNum int, hasNum bool, includeClosed bool) (string, []any) {
	...
	if !includeClosed {
		whereClause += " AND issue_effective_status(i.workspace_id, i.status) NOT IN ('done', 'cancelled') AND i.status <> 'archived'"
	}
