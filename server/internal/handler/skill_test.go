...
func TestImportAllSkillsFromRepo_TruncatedTreeStillWorks(t *testing.T) {
	...
	result, err := importAllSkillsFromRepo(t.Context(), client, "acme", "large")
	if err != nil {
...
func TestFetchFromSkillsSh_RejectsBatchURL(t *testing.T) {
	...
	_, err := fetchFromSkillsSh(context.Background(), &http.Client{}, "https://skills.sh/everyinc/compound")
	if err == nil {
...
func TestImportAllSkillsFromRepo_FiltersBinaryFiles(t *testing.T) {
	...
	result, err := importAllSkillsFromRepo(t.Context(), client, "acme", "binary")
	if err != nil {
...
func TestParseSkillsShParts_ThreeSegments(t *testing.T) {
	owner, repo, skillName, err := parseSkillsShParts("https://skills.sh/acme/skills/my-skill")
	if err != nil {
		t.Fatalf("parseSkillsShParts: %v", err)
	}
	if owner != "acme" || repo != "skills" || skillName != "my-skill" {
		t.Fatalf("got owner=%q repo=%q skillName=%q, want acme/skills/my-skill", owner, repo, skillName)
	}
}

func TestParseSkillsShParts_TwoSegments(t *testing.T) {
	owner, repo, skillName, err := parseSkillsShParts("https://skills.sh/acme/skills")
	if err != nil {
		t.Fatalf("parseSkillsShParts: %v", err)
	}
	if owner != "acme" || repo != "skills" {
		t.Fatalf("got owner=%q repo=%q, want acme/skills", owner, repo)
	}
	if skillName != "" {
		t.Fatalf("expected skillName empty for batch mode, got %q", skillName)
	}
}
...
func TestParseSkillsShParts_InvalidSegments(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, _, _, err := parseSkillsShParts(tc.url)
			if err == nil {
...
func TestImportSkillsBatchHandler_ImportsAndSkipsDuplicates(t *testing.T) {
	...
	testHandler.importSkillsBatch(first, newRequest(http.MethodPost, "/api/skills/import/batch", nil), "acme", "contract", client, workspaceUUID, creatorUUID, testUserID, testWorkspaceID)
	...
	testHandler.importSkillsBatch(second, newRequest(http.MethodPost, "/api/skills/import/batch", nil), "acme", "contract", client, workspaceUUID, creatorUUID, testUserID, testWorkspaceID)
...
func TestImportSkillsBatch_RejectsTruncatedTree(t *testing.T) {
	...
	h.importSkillsBatch(w, r, "acme", "large", client, pgtype.UUID{}, pgtype.UUID{}, "", "")
