...
// Fork: "archived" rides as an 8th pseudo-builtin status (closed-not-completed;
// see internal/issueguard). It deliberately has NO canonical category — it is
// not behaviorally equivalent to any of the 7 — so it stays out of
// canonicalOrder (seeding, ranking, and category checks remain upstream-shaped)
// while IsBuiltIn / ActiveKeys treat it as built-in so write paths accept it
// and category consumers fall back to the raw key, which fork's frontend
// status config owns the display of.
const Archived = "archived"

// IsBuiltIn reports whether key is one of the 7 canonical statuses, or the
// fork's archived pseudo-builtin.
...
func ActiveKeys(ctx context.Context, q Querier, workspaceID pgtype.UUID) ([]string, error) {
	...
	// Fork: archived is accepted by Resolve but has no catalog row, so it only
	// appears through this fill-in.
	if !seen[Archived] {
