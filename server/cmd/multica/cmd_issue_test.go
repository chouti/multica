...
func TestValidateIssueStatusArchived(t *testing.T) {
	// Since upstream MUL-6243 the CLI validates FORMAT only — membership is
	// resolved server-side against the workspace catalog (see validateIssueStatus
	// and issuestatus.Resolve). Fork's invariant: "archived" is accepted locally
	// so the CLI can keep setting/restoring it.
	...
	// Well-formed unknown keys pass locally; the server rejects them against
	// the catalog. Pin this so a future CLI-side membership check surfaces here.
	for _, s := range []string{"bogus", "archive", "human_review"} {
		if err := validateIssueStatus(s); err != nil {
			t.Errorf("validateIssueStatus(%q) = %v, want nil (format-only validation)", s, err)
		}
	}
	// Malformed keys still error and the message names the built-ins,
	// including fork's archived.
	for _, s := range []string{"", "  ", "Done!", "in progress"} {
		err := validateIssueStatus(s)
		if err == nil {
			t.Errorf("validateIssueStatus(%q) = nil, want a format error", s)
			continue
		}
