...
import { ActorMentionChip, type ActorMentionType } from "@multica/ui/components/common/actor-mention-chip";
...
function RichLink({ href, children }: { href?: string; children?: ReactNode }) {
  ...
  if (isMentionHref(href)) {
    // Generic regex: dispatch below is registry/type-driven, so new mention
    // types (skill, squad) are handled without regex edits.
    const match = href.match(/^mention:\/\/(\w+)\/(.+)$/);
    const mentionType = match?.[1];
    const mentionId = match?.[2];

    if (mentionType === "issue" && mentionId) {
      ...
      return <IssueMentionLink issueId={mentionId} label={childrenToLabel(children)} />;
    }
    if (mentionType === "project" && mentionId) {
      return <ProjectMentionLink projectId={mentionId} label={childrenToLabel(children)} />;
    }
    ...
    if (mentionType && mentionId && isActorMentionType(mentionType as MentionType)) {
      const rawLabel = childrenToLabel(children) ?? mentionId;
      ...
      return (
        <MentionHoverCard type={mentionType} id={mentionId}>
          <ActorMentionChip
            type={mentionType as ActorMentionType}
            label={label}
            initials={initials}
          />
        </MentionHoverCard>
    ...
    if (mentionType === "skill" && mentionId) {
      const rawLabel = childrenToLabel(children) ?? mentionId;
      const label = rawLabel.startsWith("@") ? rawLabel.slice(1) : rawLabel;
      return (
