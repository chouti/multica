...
vi.mock("@multica/core/issues/config", () => ({
  ...
  STATUS_CONFIG: {
    ...
    archived: { label: "Archived", iconColor: "text-muted-foreground/70", hoverBg: "hover:bg-accent" },
  },
...
describe("IssueDetail (shared)", () => {
  ...
  it("renders the issue title leaf as a link to the issue detail page", async () => {
    ...
    // the full page from the inline Inbox pane). Its text is the merged
    // `{identifier} {title}` span (kept as one node so upstream's
    // `getByText(title)` uniqueness probes resolve to the H1, not the leaf).
    await screen.findAllByText("Implement authentication");
    const leaf = screen.getByText(/TES-1\s+Implement authentication/).closest("a");
    expect(leaf).toHaveAttribute("href", "/test/issues/issue-1");

    // The fork's tappable identifier badge (status icon + copy-to-clipboard)
    // lives above the title in the body, not inside the breadcrumb link — so
    // the leaf stays a single text node. It is still rendered and still copies.
    expect(screen.getByRole("button", { name: /TES-1/ })).toBeInTheDocument();
  });
  ...
  describe("sub-issues list", () => {
    ...
    it("dims archived sub-issue titles like other closed statuses (FZG-343)", async () => {
      mockApiObj.listChildIssues.mockResolvedValue({
        issues: [
          subIssue({
            id: "child-1",
