...
function groupItems(items: MentionItem[], query: string): MentionGroup[] {
  ...
  // "Users" (member/agent/squad/all), "Issues" (issue/project), and "Skills"
  // (skill) are all produced via getMentionGroupLabel into this one Map.
  const typeGroups = new Map<string, MentionItem[]>();
  // Cancelled rows are split out by STATUS, not type: they must never precede
  // a live row, so they get their own always-last group (upstream MUL-5824).
...
function MentionRow({
  ...
  if (item.type === "issue") {
    ...
    // selectable. Archived reads as closed (FZG-343, direction A) but keeps its
  ...
  if (item.type === "skill") {
    // Skill mentions (fork #5346): wrap with upstream's disabledMessage + Tooltip
    // pattern (MUL-5559) so unbound skills stay discoverable but unselectable.
    const skillDisabled = item.disabledReason
    ...
    const skillButton = (
      <button
        type="button"
        ref={buttonRef}
        aria-disabled={skillDisabled ? true : undefined}
        aria-label={
          skillDisabled ? `${item.label}: ${skillDisabled}` : undefined
        }
        className={`flex w-full items-center gap-2.5 px-3 py-2 text-left text-caption transition-colors ${
          selected ? "bg-accent" : skillDisabled ? "" : "hover:bg-accent/50"
        } ${skillDisabled ? "cursor-not-allowed opacity-50" : ""}`}
        onClick={onSelect}
      >
        <span className="flex h-7 w-7 shrink-0 items-center justify-center">
    ...
    if (!skillDisabled) return skillButton;
    return (
      <Tooltip>
        <TooltipTrigger render={skillButton} />
