...
describe("buildIssueStatusCatalog", () => {
  ...
  it("isIssueStatusCategory accepts exactly the 8 — 7 upstream + the fork's archived", () => {
    ...
    // NOT completed), mirroring the server-side IsBuiltIn patch: board
    // bucketing and presentation lookups must keep archived issues instead
    // of dropping them (null category) or mislabeling them as todo.
